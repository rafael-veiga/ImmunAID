...
aux$col_p = get_protein_names(aux$col_p,prot)
aux = aux %>% mutate(out = factor(col_p, levels=aux$col_p))
fig_aux = aux %>% ggplot(aes(y=out, x=or, xmin=or_l, xmax=or_h)) +
  geom_pointrange()+
  scale_y_discrete (limits = rev(as.vector(aux$out)),labels=rev(as.vector(aux$out)))+
  scale_x_continuous(trans = scales::log_trans(),breaks = c(0.1,0.5,1,5,10,50,100),limits=c(NA,NA))+
...
pdf("figS13.pdf", family=font,width = 11,height = 12)
print(fig_aux)
dev.off()
