...
dis = {"Autoinflammation of unknown origin":10,"Still's disease":10,"FMF":10,"Behcet":10}

    
def rf_res():
    df = py.read_r("./data_nor_f.rds")[None]
    df["sex"]=pd.get_dummies(df["sex"]).iloc[:,0]
    df["disease"] = df["disease"].astype("category")
    catego = df["disease"].cat.categories.tolist()
    del df["treated"]
    ...
    data.to_csv("./RF_auc1_f.csv",index=False)

    df = py.read_r("./data_nor_p.rds")[None]
    df["sex"]=pd.get_dummies(df["sex"]).iloc[:,0]
    df["disease"] = df["disease"].astype("category")
    catego = df["disease"].cat.categories.tolist()
    #catego.remove("Healthy")
        ...
        for d2 in range(len(catego)):
            if d1 != d2:
                ...
                for cv, (train_index, test_index) in enumerate(skf.split(x, y)):
                    ...
                    data_aux["d2"] = catego[d2]
                    data = pd.concat([data,data_aux ], ignore_index=True)
    data.to_csv("./RF_auc1_p.csv",index=False)
    ...
    for d1 in dis:
        for d2 in dis:
            if d1!=d2:
                ...
                data = pd.concat([data, data_aux],ignore_index=True)
    data.to_csv("./RF_auc2_f.csv",index=False)

    df = pd.read_csv("./RF_auc1_p.csv")
    dis = pd.unique(list(df["d1"])+list(df["d2"]))
    data = pd.DataFrame()
    for d1 in dis:
        for d2 in dis:
            if d1!=d2:
                ...
                data_aux["par"]= tb.index[0]
                data = pd.concat([data, data_aux],ignore_index=True)
    data.to_csv("./RF_auc2_p.csv",index=False)

def rf_imp1():
    res = pd.read_csv("./RF_auc2_f.csv")
    df = py.read_r("./data_nor_f.rds")[None]
    df["sex"]=pd.get_dummies(df["sex"]).iloc[:,0]
    df["disease"] = df["disease"].astype("category")
    del df["treated"]
    ...
    data.to_csv("./RF_imp_f.csv",index=False)

    res = pd.read_csv("./RF_auc2_p.csv")
    df = py.read_r("./data_nor_p.rds")[None]
    df["sex"]=pd.get_dummies(df["sex"]).iloc[:,0]
    df["disease"] = df["disease"].astype("category")
    del df["treated"]
    ...
    data.to_csv("./RF_imp_p.csv",index=False)
...
data.to_csv("./RF_imp2_f.csv",index=False)
...
data.loc[data["vari"]!="Age"]
data.sort_values("imp",ascending=False,inplace=True)
data.to_csv("./RF_imp2_p.csv",index=False)
