...
dis = list(var = c("Healthy","Autoinflammation of unknown origin","Still's disease","FMF","Behcet"),
           ...
           pos_refy1 =c(NA,0,1.7,1,2),
           pos_refx2 = c(NA,0.8,0.4,0.9,0.5),
           pos_refy2 =c(NA,1.5,0,-0.2,-1))
...
fig_b = data %>% filter(disease==dis$var[d]) %>%
  ...
  geom_vline(xintercept=size_p,color = "darkred") +
  annotate("text",x=size_p-4,y=0.73,label=paste0("n = ",size_p),size=4,family=font,angle = 90,color = "darkred")+
...
p <- data %>% ggplot(aes(x=PC1, y=PC2))+
  ...
  theme(legend.position = "none",axis.title.x = element_text(size = 10,family = font),axis.title.y = element_text(size = 10,family = font),
        axis.text.x = element_text(size=8,family = font),axis.text.y = element_text(size=8,family = font))
fig1_pca <- p + 
  geom_richtext(data=aux, aes(x=PC1*30, y=PC2*30+c(-0.5,2), label=Disease), size = 2.5,hjust=c(dis$pos_refx1[d],dis$pos_refx2[d]),vjust=c(dis$pos_refy1[d],dis$pos_refy2[d]), color="black",family=font,fill=fill_alpha("bisque2",0.7))+
  geom_segment(data=aux,aes(x=0, y=0, xend=PC1*30, yend=PC2*30), arrow=arrow(length=unit(0.3,"cm")), color="black",linewidth=1)+
  theme_bw(base_family = font)+
  theme(legend.position = "none",axis.title.x = element_text(size = 10,family = font),axis.title.y = element_text(size = 10,family = font),
...
pdf("./figS14.pdf", family=font,width = 13.5,height = 12)
print(p)
dev.off()
