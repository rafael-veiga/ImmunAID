...
dis = {"Autoinflammation of unknown origin":40,"Still's disease":53,"FMF":45,"Behcet":112}
...
def calculate_auc_vs_n():
    ...
    data = pd.DataFrame()
    for d in catego:
        res = pd.read_csv("./mark_f/"+ d +".csv")["col_f"]
        ...
        for n in range(1, len(res)):
            ...
            aucs = Parallel(n_jobs=n_jobs)(delayed(compute_auc)(x, y, LR, i) for i in range(10))
    ...
    data.to_csv("./auc_n_f.csv",index=False)
    ...
    data = pd.DataFrame()
    for d in catego:
        res = pd.read_csv("./mark_p/"+ d +".csv")["col_p"]
        auc_list = np.empty(len(res)-1)
        auc_list[:] = np.nan
        df1 = df.copy()
        df1["y"] = np.nan
    ...
    data.to_csv("./auc_n_p.csv",index=False)
...
for d in catego:
    aux=df.copy()
    res = pd.read_csv("./mark_f/"+ d +".csv")
    res = res["col_f"][0:dis[d]]
    if d=="FMF":
    ...
    data_aux["disease"] = d
    data = pd.concat([data, data_aux],ignore_index=True)
data.to_csv("./auc_curv_f.csv",index=False)
...
for d in catego:
    aux=df.copy()
    res = pd.read_csv("./mark_p/"+ d +".csv")
    res = res["col_p"][0:dis[d]]
    if d=="FMF":
        res = ["age","disease"]+list(res)
    else:
        res = ["sex","age","disease"]+list(res)
    ...
    data_aux["disease"] = d
    data = pd.concat([data, data_aux],ignore_index=True)
data.to_csv("./auc_curv_p.csv",index=False)            
