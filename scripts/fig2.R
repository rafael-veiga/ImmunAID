...
i=2
df$out[df$disease == dis$var[i] & !is.na(df$treated) & df$treated == 0] ="treatment naïve" 
df$out[df$disease == dis$var[i] & !is.na(df$treated) & df$treated == 1] ="off-treatment"
...
p <- aux %>% ggplot(aes(x=PC1, y=PC2))+
  ...
  scale_color_manual(values=c(dis$color[1],"off-treatment"="pink3","treatment naïve"=dis$color[i][[1]]),limits = c("Healthy","off-treatment","treatment naïve"))+
...
data$Disease = y
aux =  as.data.frame(pc$rotation[,1:2])[res[1:20],]
row.names(aux) = NULL
aux$Disease = res[1:20]
...
h_t = centroid_dist(x,y,c("Healthy","off-treatment"))
h_nt = centroid_dist(x,y,c("Healthy","treatment naïve"))
t_nt = centroid_dist(x,y,c("off-treatment","treatment naïve"))

fig_d = plot_triangle_dist(
  labels = c("Healthy","off-treatment","treatment naïve"),
...
pdf("./fig2.pdf", family=font,width = 11,height = 12)
print(p)
dev.off()
