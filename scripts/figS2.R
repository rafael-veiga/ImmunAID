...
dis = list(var = c("Healthy","Autoinflammation of unknown origin","Still's disease","FMF","Behcet"),
           ...
           ref2 = c(NA,"IgM+ IgD+ Memory B Cells in Memory B Cells",
                    "HLA-DR+ CD38high CD4+ T Cells in CD4+ T Cells",
...
#figure s2
##or A
res = read.csv(paste0("./mark_f/" ,dis$var[3] ,".csv"))
...
fig_a = aux %>% ggplot(aes(y=out, x=or, xmin=or_l, xmax=or_h)) +
    ...
    geom_point(shape = 15, size = 2,color=dis$color[3])+
...
# figure D auc curve
data = read.csv(file = "./auc_curv_f.csv")
d=3
...
col = colnames(df)
col = col[col!="disease"]
df = df[df$disease %in% c("Healthy",dis$var[i]),]
...
p <- data %>% ggplot(aes(x=PC1, y=PC2))+
  ...
  theme(legend.position = "none",axis.title.x = element_text(size = 10,family = font),axis.title.y = element_text(size = 10,family = font),
        axis.text.x = element_text(size=8,family = font),axis.text.y = element_text(size=8,family = font))
fig1_pca <- p + 
  geom_richtext(data=aux, aes(x=PC1*20, y=PC2*20+c(-0.5,2), label=new_labels(Disease)), size = 2.5,hjust=c(dis$pos_refx1[i],dis$pos_refx2[i]),vjust=c(dis$pos_refy1[i],dis$pos_refy2[i]), color="black",family=font,fill=fill_alpha("bisque2",0.7))+
...
pdf("./figS2.pdf", family=font,width = 13.5,height = 12)
print(p)
dev.off()
